...
- **Open / Closed の整理** — 対応済みコメントは Closed にまとまり、
  既定で折りたたみ。Open も必要に応じて折りたためます
- **安全な一括削除** — 「すべて削除」は確認ダイアログを挟んでから実行
...
npx -y github:ykitaza/ai-review-comments --store /path/to/.ai-review/comments.json pending /abs/path/to/file.md
...
設定ドロワーでは、すべてのコピー出力の先頭に入る**共通プロンプト**と、
目的別の**個別プロンプト**を別々に編集できます。共通プロンプトには対象ファイル、
作業ルート、`.ai-review/comments.json` の場所、コメントストア内キーなどの
メタ情報が入り、AIエージェントが別ディレクトリから起動してもコメントストアを
特定しやすくなります。
対象ファイル配下に別の `.git/` がある場合は、CLI に
`--store /path/to/.ai-review/comments.json` を付けると、自動探索ではなく指定した
コメントストアを使えます。
